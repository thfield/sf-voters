var DATAFILE = 'data/pres_dem.csv' // input data file
...
queue()
  .defer(d3.json, 'data/elect_precincts-topo.json')
  .defer(d3.csv, DATAFILE) //data file here
  .await(renderFirst)

function renderFirst(error, topoData, csvData) {
  if (error) throw error;
  var TOPOBOUNDARY = 'elect_precincts'
  ...
  g.selectAll("path")
      .data(topojson.feature(topoData, topoData.objects[TOPOBOUNDARY]).features)
    .enter().append("path")
      .attr("d", path)
      .on("click", clicked)
  ...
  g.append("path")
      .datum(topojson.mesh(topoData, topoData.objects[TOPOBOUNDARY], function(a, b) { return a !== b; }))
      .attr("class", "mesh")
      .attr("d", path);
